...
class ApiRadio extends ApiResource{
    ...
    public function getIbase($ids_list){
        ...
        foreach ($ids_list as $id){
            ...
            $query =  "select r.sdate_time, r.radiation
                from rhob r
                where r.sobj = :sind  and r.sdate_time between :after and :before
                order by r.sdate_time";
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
                ...
                $tt = ['date_unix' => strtotime($row['SDATE_TIME']), 'date' => date("d.m.Y\TH:i", strtotime($row['SDATE_TIME'])), 'radiation' => $lval, 'unit' => 'uR/h', 'type' => 'auto'];
