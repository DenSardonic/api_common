...
class ApiWave extends ApiSingleResourceAccessCheck{
    // resources_is_accessed_list is all what I need
    protected $endpoint_name = 'wave';
    protected $resource_type = 'wave';
    ...
    public function getRequestedResources(){
        ...
        // sort resorces by database type
    ...
    public function mainActionIbase($resource_id){
        ...
        $query =  "select m.sobj, m.sdate_time, m.SIGNWAVEHEIGHT as sw, m.MAXWAVEHEIGHT as mw, m.PEAKWAVEPERIOD as pp 
        from magms2 m
        where m.sobj = :sind and m.sdate_time between :after and :before
        order by m.sobj, m.sdate_time";
        ...
        while ($row = $stmt->fetch()){
                ...
                $temp['singn_wave'] = $row['SW'];
                $temp['singn_wave_unit'] = "m";
                $temp['max_wave'] = $row['MW'];
                $temp['max_wave_unit'] = "m";
                $temp['peak_period_wave'] = $row['PP'];
                $temp['peak_period_unit'] = "m";
                // $temp['type'] = 'AGK';
